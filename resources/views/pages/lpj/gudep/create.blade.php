...
@section('title', 'LPJ Gugus Depan - Tambah')
    ...
    <div class="container-xxl flex-grow-1 ">
        ...
        <div class="card">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="card-header">Tambah LPJ Gugus Depan</h5>
            ...
            <div class="card-body mt-0 pt-0">
                <form id="form" class="mb-3" action="{{ route('lpj-gudep.store') }}" enctype="multipart/form-data" ,
                    method="POST">
                    @csrf
                        ...
                        <select class="form-select" id="role" aria-label="Default select example"
                            ...
                            @foreach ($proposalOption as $item)
                                <option value="{{ $item->id }}"
                                    {{ $item->id === old('proposal_gudep_id') ? 'selected' : '' }}>
                                    {{ $item->nama_kegiatan }}
                                </option>
                            @endforeach
                        </select>
