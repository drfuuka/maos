...
@section('content')
    <div class="container-xxl flex-grow-1 ">
        ...
        <h4 class="py-3 mb-4">
            <span class="text-muted fw-light">LPJ /</span> Gugus Depan
        </h4>

        <!-- Basic Bootstrap Table -->
        <div class="card">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="card-header">LPJ Gugus Depan</h5>
                <div class="card-header d-flex justify-content-end gap-3">
                    <a class="btn btn-label-primary" href="{{ route('lpj-gudep.export') }}">Export</a>
                    <a class="btn btn-primary" href="{{ route('lpj-gudep.create') }}">Tambah</a>
                </div>
            </div>
            @if (session('success'))
                <div class="px-4">
            ...
            <div class="table-responsive text-nowrap">
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        @if ($lpjGudep->count())
                            @foreach ($lpjGudep as $item)
                                <tr>
                                    <td><a
                                            href="{{ url('proposal-gudep/' . $item->proposal->id . '/edit') }}">{{ $item->proposal->nama_kegiatan }}</a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu" style="">
                                                <a class="dropdown-item" href="{{ route('lpj-gudep.edit', $item->id) }}"><i
                                                        class="ti ti-pencil me-1"></i> Edit</a>
                                                <form action="{{ route('lpj-gudep.destroy', $item->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
